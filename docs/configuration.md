...
| `MAX_CANCEL_COUNT` | int | 配置值 | 策略当日最大买单撤单次数 |
...
| `MAX_MARKET_SENTIMENT_AGE_SECONDS` | int | `30` | 市场情绪评分最大年龄，超时禁止买入 |

扫板资金只统计涨停价以内的委卖档位 `askPrice × askVol × 100`，买盘深度
不计入拉板成本。市场情绪、实时板块或资金流时间戳缺失时采用 fail-closed，不生成买入信号。
...
| ≥ 7.0 | 强势 | 正常排板，适度扫板 |
| ≥ 5.5 | 中性偏强 | 排板精选；扫板须满足情绪动态板块/领涨共振 |
| ≥ 4.0 | 中性 | 排板提高封单门槛；扫板须满足更严格的板块/领涨共振 |
| ≥ 2.5 | 弱势 | 仅排板，极高封单门槛 |
| < 2.5 | 极弱 | **停止买入** |
