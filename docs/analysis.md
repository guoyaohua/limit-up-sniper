...
使用截至目标日的真实成交账本做跨日 FIFO 配对，计算目标日实现的交易绩效指标。
这使前一交易日买入、目标日卖出的正常 A 股 T+1 交易能够取得真实成本基础。
...
加载指定日期中同时具有明确成交类型（`record_type=fill/trade`）与
`execution_status=FILLED` 的交易日志；仅受理的 `order_submission`、状态不明记录和
无法证明成交的旧版无类型日志不会进入盈亏统计：

```
output/trade_logs/{date_str}/fill_*.json
```

`load_trade_ledger(date_str)` 会读取所有不晚于目标日的可用成交目录。未来日期、
记录内未来成交日、非日期目录、委托提交记录和无法证明成交的旧日志都不会进入账本。

### 4.4 `match_buy_sell_pairs(logs) -> list`

**匹配逻辑**：
1. 按时间排序截至目标日的所有券商成交回报
2. 在同一账户、策略、股票内跨日 FIFO 匹配，正确处理 T+1、部分成交和一卖多买
3. 使用成交记录费用；缺失时按模拟账户费率保守估算佣金、过户费和印花税
4. 日报只保留目标日实现的退出和截至当日未平仓库存，避免重复统计历史已平仓收益
5. 无法从现有历史账本找到买入的卖出标为期初持仓/数据缺失，不虚构收益
6. 决策漏斗分别报告发单事件和券商真实成交回报，不把提交数当成交数
