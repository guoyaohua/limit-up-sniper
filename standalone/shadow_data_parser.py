"""
Shadow Data Parser Module
用于解析shadow shared data的各种字段
"""
...
class ShadowDataParser:
    ...
    @staticmethod
    def parse_shadow_data(shadow_data: Dict) -> Dict:
        """
        解析完整的shadow data
        ...
        try:
            # Parse each field
            field_parsers = {
                '股票信息': ShadowDataParser.parse_stock_info,
                '持仓状态': ShadowDataParser.parse_positions,
                '委托状态': ShadowDataParser.parse_orders,
                '昨日首板股票': ShadowDataParser.parse_stock_list,
                '昨日涨停股票': ShadowDataParser.parse_stock_list,
                '涨停池': ShadowDataParser.parse_timestamp_dict,
                '炸板池': ShadowDataParser.parse_timestamp_dict,
                '最大开板回封时间': ShadowDataParser.parse_number_dict,
                '开板次数': ShadowDataParser.parse_number_dict,
                '股票状态信号': ShadowDataParser.parse_stock_signals,
                '黑名单': ShadowDataParser.parse_blacklist,
                '强势股票': ShadowDataParser.parse_stock_list,
            }

            for field_name, parser_func in field_parsers.items():
                if field_name in shadow_data:
                    try:
                        parsed[field_name] = parser_func(
                            shadow_data[field_name])
                        logger.debug(f"成功解析字段 {field_name}")
                    except Exception as e:
                        logger.error(f"解析字段 {field_name} 失败: {e}")
                        parsed[field_name] = shadow_data.get(field_name, {})
            ...
            logger.error(f"解析shadow data失败: {e}")
